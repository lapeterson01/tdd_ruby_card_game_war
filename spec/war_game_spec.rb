...
describe 'WarGame' do
    ...
    describe '#play_round' do
        it 'compares a card from each of the players hand and adds both cards to the hand of the player that won the round' do
            ...
            player1.retrieve_card(card1) and player2.retrieve_card(card2)
        ...
        it 'sends all four cards to the hand of the winning player if the previous round resulted in a tie' do
            card1, card2, card3, card4 = PlayingCard.new('Q', 'Spades'), PlayingCard.new('Q', 'Hearts'), PlayingCard.new('A', 'Spades'), PlayingCard.new('2', 'Clubs')
            player1, player2 = game.player1, game.player2

            player1.retrieve_card(card1) and player2.retrieve_card(card2)
            player1.retrieve_card(card3) and player2.retrieve_card(card4)
            game.play_round
    ...
    describe '#winner' do
        ...
        it 'assigns winner to the player when the opposing player runs out of cards even in the case of a tie on the last round' do
            player1, player2 = game.player1, game.player2
